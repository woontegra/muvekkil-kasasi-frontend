...
import { authCardVariants, transition } from '../../motion/variants'
...
export function AuthFormCard({ title, subtitle, icon, children, footer, className, wide }: AuthFormCardProps): ReactElement {
  ...
  const t = transition(reducedMotion, 'slow')

  return (
    <motion.div
      variants={reducedMotion ? undefined : authCardVariants}
    ...
    >
      <motion.div
        ...
        transition={reducedMotion ? { duration: 0 } : { ...t, delay: 0.12 }}
      >
        {ICONS[icon]}
      </motion.div>
