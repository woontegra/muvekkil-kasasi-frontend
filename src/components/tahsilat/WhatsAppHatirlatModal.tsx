...
import { getWhatsAppBaglantiDurum } from '../../api/whatsappBaglanti'
...
async function sendJobViaCloud(jobId: string, mesaj: string): Promise<{ ok: true; jobId: string }> {
  return apiFetch(`/api/v1/tahsilat-bildirim/isler/${encodeURIComponent(jobId)}/whatsapp-cloud-gonder`, {
    method: 'POST',
...
export function WhatsAppHatirlatModal({ row, onClose }: Props): ReactElement {
  ...
  const baglantiQ = useQuery({
    queryKey: ['whatsapp-baglanti', 'durum', 'modal'],
    queryFn: getWhatsAppBaglantiDurum,
    staleTime: 30_000
  })
  const cloudReady = Boolean(baglantiQ.data?.connected && baglantiQ.data?.gercekGonderimAktif)
  ...
  const cloudMu = useMutation({
    mutationFn: async () => {
      const prepared = await prepareManualWhatsApp(row.id, {
        mesaj,
        idempotencyKey: `ui-cloud-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
        openDeepLink: false
      })
      if (!prepared.jobId) throw new Error('Bildirim kaydı oluşturulamadı.')
      setJobId(prepared.jobId)
      return sendJobViaCloud(prepared.jobId, mesaj)
    },
    onSuccess: () => {
      toast.success('WhatsApp API ile gönderildi.')
      invalidateTahsilatBildirim(qc)
      onClose()
    },
    onError: (e) => {
      toast.error(friendlyClientErrorMessage(e, 'API gönderimi başarısız.'))
    }
  })
  ...
  const busy = prepareMu.isPending || cloudMu.isPending || markMu.isPending
    ...
    <ModalScrim onClose={onClose} wide innerAsDialog>
      <div className="mx-auto w-full max-w-lg rounded-xl border border-border bg-panel p-5 shadow-xl">
        ...
        <div className="space-y-3">
          <AlertBox variant="info" title="Gönderim yöntemi">
            {cloudReady
              ? 'WhatsApp Business bağlı. API ile gönderebilir veya WhatsApp’ta manuel açabilirsiniz.'
              : 'Hatırlatma metni hazırlanır; gönderim kendi WhatsApp hesabınızdan (wa.me) tamamlanır. API için Ayarlar → WhatsApp’tan bağlanın.'}
          </AlertBox>
            ...
            <>
              ...
              <div className="flex flex-wrap justify-end gap-2 pt-1">
                ...
                {whatsappAcildi && jobId ? (
                  <Button type="button" variant="outline" onClick={() => markMu.mutate()} disabled={busy}>
                ...
                <Button
                  ...
                  disabled={busy || !mesaj.trim() || !telefonGecerli}
                >
                  {prepareMu.isPending ? 'Açılıyor…' : "WhatsApp'ta manuel aç"}
                </Button>
                {cloudReady ? (
                  <Button
                    type="button"
                    onClick={() => cloudMu.mutate()}
                    disabled={busy || !mesaj.trim() || !telefonGecerli}
                  >
                    {cloudMu.isPending ? 'Gönderiliyor…' : 'WhatsApp API ile gönder'}
                  </Button>
                ) : null}
              </div>
            </>
