                ...
                {items.map((row) => (
                  <MobileRecordCard
                    ...
                    subtitle={
                      <>
                        {row.dosyaBaslik}
                        {row.dosyaNo?.trim() ? ` · No: ${row.dosyaNo}` : ''}
                        {` · ${dosyaTuruEtiket(row.dosyaTuru)}`}
                      </>
                    }
                    badge={
                    ...
                    fields={[
                      { label: 'Tahsilat tarihi', value: formatDateTR(row.tahsilatTarihi) },
                      { label: 'Tutar', value: formatCurrencyTR(Number(row.tutar)), numeric: true },
                      { label: 'Tahsilat türü', value: row.tahsilatTuru },
                      { label: 'Ödeme', value: row.odemeYontemi?.trim() || '—' },
                      { label: 'Belge no', value: row.belgeNo?.trim() || '—', full: true }
                    ]}
                    actions={
                      <MobileActionBar
                        items={[
                          {
                            ...
                            primary: true,
                            variant: 'outline',
                            onClick: () => navigate(dosyaHref(row.muvekkilId, row.dosyaId))
                          },
                          {
                            key: 'smm',
                            label: 'SMM kesildi',
