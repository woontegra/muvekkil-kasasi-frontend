...
import { authHeroContainer, authHeroItem, transition } from '../motion/variants'
  ...
  return (
    <div className="relative min-h-screen min-h-[100dvh] text-ink">
      ...
      <div className="pointer-events-none fixed inset-0 z-0" aria-hidden>
        ...
        />
        <motion.div
          className="absolute -left-20 top-[18%] h-[280px] w-[280px] rounded-full bg-[#3b82f6] opacity-55 blur-[52px]"
          initial={reducedMotion ? false : { opacity: 0, scale: 0.85 }}
          animate={{ opacity: 0.55, scale: 1 }}
          transition={reducedMotion ? { duration: 0 } : { duration: 1.1, ease: [0.22, 1, 0.36, 1] }}
        />
        <motion.div
          className="absolute -right-10 bottom-[12%] h-[220px] w-[220px] rounded-full bg-[#06b6d4] opacity-55 blur-[52px]"
          initial={reducedMotion ? false : { opacity: 0, scale: 0.85 }}
          animate={{ opacity: 0.55, scale: 1 }}
          transition={reducedMotion ? { duration: 0 } : { duration: 1.2, delay: 0.15, ease: [0.22, 1, 0.36, 1] }}
        />
      </div>
      ...
      <div className="relative z-10 mx-auto grid min-h-screen max-w-[1080px] grid-cols-1 content-start gap-10 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)] lg:content-center lg:gap-x-10 lg:gap-y-8 lg:py-12 xl:max-w-[1120px]">
        <motion.section
          className="flex min-h-0 flex-col pt-1 lg:max-w-xl lg:justify-center"
          aria-labelledby="auth-hero-title"
          variants={reducedMotion ? undefined : authHeroContainer}
          initial={reducedMotion ? false : 'initial'}
          animate="animate"
        >
          <motion.div
            variants={reducedMotion ? undefined : authHeroItem}
            transition={t}
            className="mb-5 inline-flex w-max max-w-full items-center gap-2 rounded-full border border-cyan-600/25 bg-white/60 px-3 py-1.5 text-[0.68rem] font-bold uppercase tracking-[0.12em] text-cyan-800 shadow-[inset_0_1px_0_rgba(255,255,255,0.9)] backdrop-blur-sm"
          >
          ...
          <motion.div variants={reducedMotion ? undefined : authHeroItem} transition={t} className="mb-5">
            <img
              src={PROGRAM_LOGO_SRC}
              alt=""
              className="block h-auto max-h-[7.5rem] w-auto max-w-[min(220px,88vw)] object-contain object-left"
            />
          </motion.div>
          ...
          <motion.p
            variants={reducedMotion ? undefined : authHeroItem}
            transition={t}
            className="mb-6 max-w-[38ch] text-[0.98rem] leading-relaxed text-ink-muted"
          >
          ...
          <motion.div
            variants={reducedMotion ? undefined : authHeroItem}
            transition={t}
            className="mt-10 flex flex-wrap items-center gap-2.5 border-t border-slate-400/30 pt-4 lg:mt-auto"
          >
